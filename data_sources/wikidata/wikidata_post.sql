...
CREATE MATERIALIZED VIEW wikidata AS
    WITH data AS (
        SELECT 
            uid, source_id, name, gadm2, 'wikidata' AS data_source, the_geom
        FROM 
            wikidata_records
        UNION
        SELECT 
            r.uid, r.source_id, n.name, r.gadm2, 'wikidata' AS data_source, r.the_geom
        FROM 
    ...
    SELECT uid, source_id, name, gadm2, data_source, the_geom FROM data WHERE name IS NOT NULL GROUP BY uid, source_id, name, gadm2, data_source, the_geom;
