...
#Recreate indices
psql -U gisuser -h localhost osm -c "CREATE INDEX osm_name_idx ON osm USING gin (name gin_trgm_ops);"
psql -U gisuser -h localhost osm -c "CREATE INDEX osm_country_idx ON osm USING gin (country gin_trgm_ops);"
psql -U gisuser -h localhost osm -c "CREATE INDEX osm_gadm2_idx ON osm USING gin (gadm2 gin_trgm_ops);"
